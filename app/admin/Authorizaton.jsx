                ...
                <div className="d-flex">

                    {
                        Object.entries(action).map(([key, value]) => {
                            // console.log("isChecked:", isChecked.includes(key))
                            return (<>
                                <label key={key} className="form-check form-check-sm form-check-custom form-check-solid me-5 me-lg-20">
                                    <input
                                        id={`${id}_${key}`}
                                        data-parent={id}
                                        className={`form-check-input role_${id}`}
                                        type={type}
                                        onChange={handleClick}
                                        value={key}
...
export default function Authorization({ params }) {
    const modulesData = [
        ...
        },
        {
            ...
            "action": { 1: "Read", 2: "Write", 3: "Create", 4: "Hostory" }
        },
        {
            "id": "04",
            "name": "Reporting",
    ...
    const handleSelectAll = e => {
        ...
        Object.keys(isCheck).forEach((checkedKey) => {
            let getBox = document.querySelectorAll(`.role_${checkedKey}`);
            let checkedData = [];
            if (e.target.checked) {
                getBox.forEach((row) => { checkedData.push(row.value); });
            }
            setIsCheck((prevData) => {
                return {
                    ...prevData,
                    [checkedKey]: checkedData
                }
            });
        })
    ...
    const handleClick = e => {
        const { id } = e.target;
        const dadicated_ids = id.split("_")
        let getBox = document.querySelectorAll(`.role_${dadicated_ids[0]}:checked`);
        let checkedData = [];
        getBox.forEach((row) => { checkedData.push(row.value); });
        setIsCheck((prevData) => {
            return {
                ...prevData,
                [dadicated_ids[0]]: checkedData
            }
        });
    };
    ...
    const submitPermission = (e) => {
        e.preventDefault();
        console.log("Submit data", isCheck)
    }
    return (<>
        <div
            ...
            <div className="content flex-row-fluid" id="kt_content">
                ...
                <div className="card card-flush ">
                    ...
                    <div className="card-body pt-0">
                        <form id="kt_modal_update_role_form" onSubmit={submitPermission} className="form" action="#">
                            {/*begin::Scroll*/}
                            <div
